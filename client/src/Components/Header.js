...
export default function Header({ isAuthenticated, setIsAuthenticated }) {
    ...
    return (
        <div className="fixed top-0 w-full z-200 bg-header px-[15px] h-[38px] flex justify-between items-start py-1">
            <div className="bg-[#3b3b4f] h-full flex flex-grow-0 flex-shrink-0 w-1/3 text-white ">
            ...
            <div className='flex flex-grow-0 flex-shrink-0 w-1/3 justify-end gap-[10px] h-full'>
                <div className='text-white border border-1 border-white cursor-pointer items-center flex text-2xl px-1 hover:text-black hover:bg-white'>
                    <VscGlobe />
                </div>
                <button className='border border-1 border-white text-white px-4 hover:text-black hover:bg-white'>Menu</button>
